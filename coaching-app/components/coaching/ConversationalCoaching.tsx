...
export default function ConversationalCoaching() {
  ...
  useEffect(() => {
    ...
    // ÖNCE kontrol et - bu stage zaten initialize edilmiş mi?
    if (initializedStage.current === currentStage) {
      console.log('⏭️ Stage already initialized, skipping');
      return;
    }

    // ÖNCE mark et - React Strict Mode'da 2. çağrıyı engelle
    ...
    // Async function içinde cancelled kontrolü yap
    const initStage = async () => {
      if (cancelled) {
        console.log('⏭️ Effect was cleaned up, skipping API call');
        return;
      }
      await sendInitialMessage(initialState);
    };

    initStage();

    // Cleanup function: component unmount olursa veya stage değişirse
    return () => {
      console.log('🧹 Cleaning up useEffect for stage:', currentStage);
      cancelled = true;
    };
  }, [currentStage]);
  ...
  const sendInitialMessage = async (state: CoachingState) => {
    ...
    // Eğer bu stage zaten initialize edildiyse, çıkış yap!
    if (initializedStage.current === state.stage) {
      console.log('⏭️ Stage already initialized in sendInitialMessage, skipping');
      return;
    }
    ...
    try {
      ...
      if (data.state.stage !== coachingState.stage) {
        ...
        if (data.state.stage === 2 && coachingState.stage > 2) {
          ...
          setAllMessages(prev => [...prev, assistantMsg]);
          return; // ÇOK ÖNEMLİ: Burada dur, setCoachingState çağrılmasın!
        }
